...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<% ArrayList<BoardFree> list=(ArrayList)request.getAttribute("list"); 
...
window.onload=function(){
    ...
    var scon=document.querySelector("#search-userContent");
...
div#search-usertitle {
    ...
    <div class="container">
        ...
        <div style="text-align: center">
                 <select id="searchType" class="pagination">
            <option value="usertitle" <%="usertitle".equals(searchType)?"selected":""%>>제목</option>      
            <option value="userId" <%="userId".equals(searchType)?"selected":""%>>작성자</option>
            <option value="userContent" <%="userContent".equals(searchType)?"selected":""%>>내용</option>
        </select>
        <div id="search-usertitle">
            <form action="<%=request.getContextPath()%>/boardFreeFinder">
               ...
               <input type='text' name="searchKeyword" size='25' placeholder="검색어를 입력하세요." style="width: 300px" 
                value="<%="usertitle".equals(searchType)?searchKeyword:""%>" />
          ...
          <div id="search-userContent">
            <form action="<%=request.getContextPath()%>/boardFreeFinder">
                ...
                <input type="text" name="searchKeyword" size="25" placeholder="검색어를 입력하세요." style="width: 300px"
                    value="<%="userContent".equals(searchType)?searchKeyword:""%>"/>
                <button class="btn btn-primary" type='submit' style="width:60px">검색</button>
            </form>   
        </div>     
        </div>
